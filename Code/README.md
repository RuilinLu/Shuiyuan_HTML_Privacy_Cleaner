...
V6 的关键变化：全匿名模式不再提取发言后重写一个新页面，而是直接在原始 SingleFile/Discourse HTML 上就地匿名化，尽量保留原版论坛布局、楼层、引用、时间线和静态阅读体验。

本次 V6 更新还修复了三个公开分发前发现的问题：

- 全匿名模式会在每个帖子头像旁和帖子头部显示一致的 `用户1`、`用户2` 等匿名编号，并用内嵌 SVG 虚拟头像替换原头像。
- 用户名替换会先保护 `data:image/...` 资源，避免误改 Base64 导致正文图片无法渲染。
- 全匿名模式会移除底部推荐/相关话题区和登录交互页脚，避免额外站点信息残留，也避免静态快照里推荐列表错位。
...
- 会给帖子头部和头像区补充一致的匿名编号；原始头像会替换为本地生成的虚拟头像，不再保留原头像 CSS 图片变量。
- 会移除所有用户徽章、头衔、flair 节点，不限定于某几个徽章名称。
- 会删除 Discourse 预加载 JSON 和站点外链，避免隐藏数据中残留原始用户、图片 URL 或站点域名。
- 会删除底部“推荐/相关”话题列表、通知按钮等与当前正文无关的登录态/推荐信息。
...
- 最新全匿名回归样本：21 个帖子均有可见匿名编号；头像均为内嵌 SVG 虚拟头像；正文 PNG Base64 校验通过；Edge 截图显示正文图片正常渲染；底部推荐区已移除，不再错位。
...
严格说明：任何工具都不能数学证明未知隐写或任意构造隐藏信息 100% 不存在。V6 的审核覆盖了当前已知的 SingleFile/Discourse 水印、隐水印、登录态、预加载数据、用户标识、外链和站点痕迹。
...
- `APP/V6`：在 V5 基础上补充匿名头像、帖子头部编号、图片 Base64 保护和推荐区移除，并更新三架构发布包。
